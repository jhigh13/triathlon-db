# Para Standards Batch Report Generation

This directory contains tools to generate para triathlon standards reports for all categories automatically.
...
- Save outputs to timestamped directory: `tri_analysis/outputs/para_standards_batch_YYYYMMDD_HHMMSS/`
...
```powershell
python convert_reports_to_pdf.py tri_analysis/outputs/para_standards_batch_YYYYMMDD_HHMMSS
```
...
```powershell
python -m tri_analysis.para_standards --category "PTVI Men" --since 2021-01-01 --no-png
```

Or auto-select first athlete:

```powershell
python -m tri_analysis.para_standards --category "PTVI Men" --since 2021-01-01 --no-png --auto-select-first
```
