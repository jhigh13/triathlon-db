...
$PYTHON = "C:/Users/johnk/VSCode/triathlon-db/.venv/Scripts/python.exe"
$CATEGORIES = @(
    "PTWC Men",
    "PTS2 Men",
    "PTS3 Men",
...
$BASE_OUTPUT_DIR = "tri_analysis\outputs\para_standards_batch_$TIMESTAMP"
...
if (Test-Path "convert_reports_to_pdf.py") {
    try {
        & $PYTHON convert_reports_to_pdf.py "$BASE_OUTPUT_DIR"
        if ($LASTEXITCODE -eq 0) {
